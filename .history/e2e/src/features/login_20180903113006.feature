Feature: Login Page
    ...
    And the user set the email to 'daniel@correocom'
    And the user set the password to '12345'
    When the user click in login button 
    Then the user should see the correct message

    Scenario: HU: 1456 - Mostrar mensaje de error si el correo es valido
    Given the user is in login Page
    And the user set the email to 'danielcorreocom'
    And the user set the password to '12345'
    When the user click in login button 
    Then the user should see the correct message